...
public class MeasureActivity extends Activity 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		startMeasure.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				isRecorded = false;
				if (mSocket == null) {
					Toast.makeText(MeasureActivity.this, "蓝牙未连接", Toast.LENGTH_SHORT).show();
		...
		record.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// if already record
				if (isRecorded) {
					Toast.makeText(MeasureActivity.this, "重复记录", Toast.LENGTH_SHORT).show();
					return;
				} else if (measuredDbp == -1) {
					Toast.makeText(MeasureActivity.this, "还未测量", Toast.LENGTH_SHORT).show();
					return;
				} else {
					Toast.makeText(MeasureActivity.this, "记录成功", Toast.LENGTH_SHORT).show();
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {
					if (!isPrintNotFoundMessage && mDevice == null) {
						Toast.makeText(MeasureActivity.this, "没有找到匹配设备", Toast.LENGTH_SHORT).show();
						mAdapter.cancelDiscovery();
						endProgressDialog();
	...
	public void obtainData(int dbpValue, int sbpValue, int heartRate) {
		...
		if (dbpValue < 0 || sbpValue < 0 || heartRate < 0) {
			Toast.makeText(MeasureActivity.this, "体动, 测量失败, 请重新测量", Toast.LENGTH_SHORT).show();
			return;
		}
	...
	public void socketErrorHandler() {
		...
		Toast.makeText(MeasureActivity.this, "蓝牙连接已断开，请检测设备",	Toast.LENGTH_SHORT).show();
	...
	public void startProgressDialog(String message) {
		dialog = ProgressDialog.show(MeasureActivity.this, "请等待...", message);
	}
